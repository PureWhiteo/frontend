<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="绘本名称" prop="huibenmingcheng">
          <el-input v-model="ruleForm.huibenmingcheng" 
              placeholder="绘本名称" clearable  :readonly="ro.huibenmingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="绘本名称" prop="huibenmingcheng">
              <el-input v-model="ruleForm.huibenmingcheng" 
                placeholder="绘本名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="绘本类型" prop="huibenleixing">
          <el-select :disabled="ro.huibenleixing" v-model="ruleForm.huibenleixing" placeholder="请选择绘本类型">
            <el-option
                v-for="(item,index) in huibenleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="绘本类型" prop="huibenleixing">
	      <el-input v-model="ruleForm.huibenleixing"
                placeholder="绘本类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.huibentingdu" label="绘本听读" prop="huibentingdu">
            <audio
              :style='{"width":"300px","height":"54px"}'
              :src="$base.url+ruleForm.huibentingdu"
              controls="controls"
            >
              您的浏览器不支持音频播放
            </audio>
            <!-- <el-button type="text" size="small" @click="download($base.url+ruleForm.huibentingdu)">预览</el-button> -->
          </el-form-item>
        </div>    
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="绘本阅读" prop="huibenyuedu">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.huibenyuedu" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.huibenyuedu" label="绘本阅读" prop="huibenyuedu">
                    <span v-html="ruleForm.huibenyuedu"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="绘本译文" prop="huibenyiwen">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.huibenyiwen" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.huibenyiwen" label="绘本译文" prop="huibenyiwen">
                    <span v-html="ruleForm.huibenyiwen"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='huibenmingcheng'){
            this.ruleForm.huibenmingcheng = obj[o];
	    this.ro.huibenmingcheng = true;
            continue;
          }
          if(o=='fengmian'){
	    ...
	    this.ro.fengmian = true;
          ...
          if(o=='huibenleixing'){
            this.ruleForm.huibenleixing = obj[o];
	    this.ro.huibenleixing = true;
            continue;
          }
          if(o=='zuozhe'){
	    ...
	    this.ro.jibie = true;
          ...
          if(o=='huibentingdu'){
            this.ruleForm.huibentingdu = obj[o];
	    this.ro.huibentingdu = true;
            continue;
          }
          if(o=='huibenyuedu'){
            this.ruleForm.huibenyuedu = obj[o];
	    this.ro.huibenyuedu = true;
            continue;
          }
          if(o=='huibenyiwen'){
            this.ruleForm.huibenyiwen = obj[o];
	    this.ro.huibenyiwen = true;
            continue;
          }
          if(o=='fabushijian'){
	...
	this.ruleForm.huibenyuedu = this.ruleForm.huibenyuedu.replace(reg,'../../../springboot1144o/upload');
	this.ruleForm.huibenyiwen = this.ruleForm.huibenyiwen.replace(reg,'../../../springboot1144o/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `huibenyuedu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.huibenyueduCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    huibentingduUploadChange(fileUrls) {
	this.ruleForm.huibentingdu = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
